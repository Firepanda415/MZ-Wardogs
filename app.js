import { maps, tileBounds, weapons, parseCoordinate, validPoint, solution, heading, screenToWorld } from './core.mjs';

const $ = id => document.getElementById(id);
const strings = {
  zh: { saved:'收藏',distance:'距离',bearing:'罗盘方位',map:'地图',weapon:'武器',range:'射程',browse:'浏览',place:'放置目标',expand:'展开地图',collapse:'返回计算',origin:'自己',target:'目标',inputOnly:'仅输入',inputOrMap:'输入或点选',lock:'锁定',locked:'已锁定',clear:'清空',save:'保存',name:'给坐标起个名字',about:'使用说明',coordinateHint:'游戏坐标 · 0.01 = 1 m',localNote:'仅保存在此浏览器，清除网站数据会删除收藏。',help1:'先输入自己坐标，再输入目标，或切换「放置目标」点选地图。锁定后需先解锁才能修改或清空。',help2:'实线圈是最大射程，虚线圈内是过近区域。距离是平面距离，射程数据为社区参考值。',help3:'浏览模式：拖动、双指或滚轮缩放。键盘：方向键移动，+/− 缩放；放置模式下 Enter 放在地图中心。',help4:'展开地图可专心看图，点位与缩放会保留。收藏按地图分别保存，可载入自己或目标。',attribution:'地图及射程参考',unofficial:'非官方玩家工具。WARDOGS 地图与商标归 BULKHEAD / 相应权利人所有。地图需要联网加载。',in:'射程内',near:'过近 · 小于最小射程',far:'超出最大射程',empty:'输入自己与目标坐标',same:'同一点 · 无方位',browseHint:'拖动浏览 · 双指 / 滚轮缩放',placeHint:'地图已固定 · 点选目标',lockedHint:'目标已锁定 · 解锁后可放置',invalid:'请输入范围内坐标（最多 2 位小数）',noSaved:'这张地图还没有收藏',loadOrigin:'设为自己',loadTarget:'设为目标',remove:'删除',undo:'撤销',deleted:'已删除收藏',savedDone:'坐标已收藏',storageError:'浏览器无法保存数据；本次操作仍可使用。',storageCorrupt:'本地数据无法读取，原数据已保留。',unlockFirst:'请先解锁这个坐标',outside:'请在地图边界内放置目标',loading:'地图加载中…',mapError:'部分地图未能加载，请检查网络',retry:'重试',yourX:'自己 X',yourY:'自己 Y',targetX:'目标 X',targetY:'目标 Y',saveOrigin:'收藏自己坐标',saveTarget:'收藏目标坐标',zoomIn:'放大',zoomOut:'缩小',fit:'全图',close:'关闭',mapLabel:'地图：方向键浏览，加减号缩放；放置模式按回车选择中心点',rangeCircle:'最大射程',minCircle:'最小射程',badFields:'请检查坐标输入' },
  en: { saved:'Saved',distance:'DISTANCE',bearing:'BEARING',map:'MAP',weapon:'WEAPON',range:'RANGE',browse:'Browse',place:'Place target',expand:'Expand map',collapse:'Back to calculator',origin:'You',target:'Target',inputOnly:'Type only',inputOrMap:'Type or tap',lock:'Lock',locked:'Locked',clear:'Clear',save:'Save',name:'Name this position',about:'How to use',coordinateHint:'Game coordinates · 0.01 = 1 m',localNote:'Saved only in this browser. Clearing site data removes saved positions.',help1:'Enter your position, then enter a target or switch to Place target and tap the map. Unlock a position before editing or clearing it.',help2:'The solid circle is maximum range. The dashed circle marks the minimum range. Distance is horizontal; weapon ranges are community reference values.',help3:'Browse: drag, pinch or scroll to zoom. Keyboard: arrows to pan, +/− to zoom; Enter places the target at the map center in placement mode.',help4:'Expand the map to browse with your positions and zoom preserved. Saved positions are grouped by map and can be loaded as you or the target.',attribution:'Maps and range data',unofficial:'Unofficial fan tool. WARDOGS maps and trademarks belong to BULKHEAD / their respective owners. Maps require an internet connection.',in:'Within range',near:'Too close · below minimum',far:'Beyond maximum range',empty:'Enter your position and target',same:'Same position · no bearing',browseHint:'Drag to pan · pinch / scroll to zoom',placeHint:'Map fixed · tap to place target',lockedHint:'Target locked · unlock to place',invalid:'Enter coordinates within bounds (up to 2 decimals)',noSaved:'No saved positions on this map',loadOrigin:'Set as you',loadTarget:'Set as target',remove:'Delete',undo:'Undo',deleted:'Position deleted',savedDone:'Position saved',storageError:'Browser storage unavailable; this session still works.',storageCorrupt:'Saved data could not be read. Original data preserved.',unlockFirst:'Unlock this position first',outside:'Place the target within the map boundary',loading:'Loading map…',mapError:'Some map images could not load. Check your connection.',retry:'Retry',yourX:'Your X',yourY:'Your Y',targetX:'Target X',targetY:'Target Y',saveOrigin:'Save your position',saveTarget:'Save target position',zoomIn:'Zoom in',zoomOut:'Zoom out',fit:'Fit map',close:'Close',mapLabel:'Map: arrow keys to pan, plus/minus to zoom; Enter places a target at the center in placement mode',rangeCircle:'Maximum range',minCircle:'Minimum range',badFields:'Check coordinate inputs' },
};
...
function persist() {
  ...
  try { localStorage.setItem(storageKey, JSON.stringify({ version:1,lang,mapId,weaponId,perMap,saved })); }
...
function updateMode() {
  ...
  $('map-panel').classList.toggle('placing',mode === 'place');
  $('map-hint').textContent = t(mode === 'browse' ? 'browseHint' : current().target.locked ? 'lockedHint' : 'placeHint');
  for (const id of ['zoom-in','zoom-out','fit']) $(id).disabled = mode === 'place';
}
function translate() {
...
const tileURL = (z,x,y) => `https://assets.wardogs-artillery.com/releases/assets-v1/maps/tiles/${mapId}/zoom_${z}/${x}_${y}.webp`;
function mapNotice() {
...
function renderMap() {
  ...
  if(mode==='place') overlays.append(svg('path',{d:`M${x-7/s} ${-y}h${14/s}M${x} ${-y-7/s}v${14/s}`,stroke:'#fff9','stroke-width':1,'vector-effect':'non-scaling-stroke'}));
...
function finishPointer(event,cancel=false){
  ...
  const place=!cancel&&mode==='place'&&g&&!g.multi&&!g.moved&&pointers.size===1&&Math.hypot(p.x-g.x,p.y-g.y)<=7;
  pointers.delete(event.pointerId);
  if(place){const world=screenToWorld(p.x,p.y,camera);setPoint('target',{x:Math.round(world.x*100)/100,y:Math.round(world.y*100)/100});}
...
canvas.addEventListener('keydown',event=>{
  ...
  if(mode==='place') {if(event.key==='Enter')setPoint('target',{x:Math.round(camera.x*100)/100,y:Math.round(camera.y*100)/100});return;}
