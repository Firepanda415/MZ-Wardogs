...
module.exports=async({mapWindow:map,sightWindow:sight,command,state,allowedFile,root,app,hotkeyPath})=>{
  ...
  const restored=['preset:bakurani:tower:2','preset:bakurani:tower:3','preset:ozeti:tower:3','preset:ozeti:tower:4','preset:zestafona:tower:2'];
  const church='preset:bakurani:southwest-church';
  assert.deepEqual(upgraded.saved.map(p=>p.id),['custom',...restored,'preset:ozeti:stadium','preset:ozeti:pool-diving-platform',church],'Upgrade must preserve custom favorites and deleted older presets');
  assert.equal(upgraded.defaultSavedVersion,4);
  ...
  await map.loadURL('wardogs://app/index.html');await settle();
  assert.deepEqual(await check(map,"JSON.parse(localStorage.getItem('mz-wardogs-v1')).saved.map(p=>p.id)"),['custom',...restored,'preset:ozeti:pool-diving-platform',church],'Deleted Stadium must stay deleted after reload');
  ...
  await map.loadURL('wardogs://app/index.html');await settle();
  assert.deepEqual(await check(map,"JSON.parse(localStorage.getItem('mz-wardogs-v1')).saved.map(p=>p.id)"),['custom',...restored,'preset:ozeti:pool-diving-platform',church],'Version 2 adds the new landmarks and previously excluded towers');
  ...
  await map.loadURL('wardogs://app/index.html');await settle();
  assert.deepEqual(await check(map,"JSON.parse(localStorage.getItem('mz-wardogs-v1')).saved.map(p=>p.id)"),['custom',...restored,church],'Deleted pool preset must stay deleted');
  await check(map,`localStorage.setItem('mz-wardogs-v1',JSON.stringify({version:1,lang:'zh',mapId:'bakurani',defaultSavedVersion:3,saved:[{id:'custom',name:'Camp',mapId:'bakurani',x:79.16,y:67.47}]}))`);
  await map.loadURL('wardogs://app/index.html');await settle();
  assert.deepEqual(await check(map,"JSON.parse(localStorage.getItem('mz-wardogs-v1')).saved.map(p=>p.id)"),['custom',...restored],'Version 3 restores excluded towers without duplicating the custom church or resurrecting deleted presets');
  assert(await check(map,"!!document.querySelector('[data-landmark=southwest-church]')&&!document.querySelector('[data-control-zone]')"),'Southwest Church appears with control-zone boundaries hidden');
  assert(await check(map,"[...document.querySelectorAll('[data-tower] rect')].every(p=>p.getAttribute('fill')==='#e8d79b')"),'All towers use their normal color');
